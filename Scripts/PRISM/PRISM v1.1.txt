...
WB_counter := 2
test := 10
veto = [System],[CloudGnome],00:00,: ANNOUNCEMENT:,more votes are needed to pass,*** player: [,** Note,<img class,<img id=, votekick ,before the game will automatically
...
Warning: Do NOT manually edit the following two options: L_Username and L_Password
...
Choose:
{
	...
	TrayTip, PRISM, User Specific works only for Island Defense!, 3, 1
}
return
...
goAll:
{
	...
	Menu, MyMenuBar, Add, Reload, iniread
...
StartAll:
{
	...
	If (more != "" && pageInc = "1")
	{
		Loop, %sites%
		{
			wb%WB_counter%.Navigate(more . sites . "/")
			GoSub, wait
	...
	countIDs := 0
		...
		{
			MsgBox,52,Information,Due to server complications`, you'll have to choose whether you wish to have Legion TD Mega games or Legion TD Beta games filtered.`n`nPress Yes for LTD Mega and No for LTD Beta.
	...
	Loop, parse, IDs,`n,`n
	{
		...
		*/
		...
		GuiControl, 2:, Static4,% countIDs " : " counterIDs2 " - max: " index
...
start:
{
	...
	Loop, parse, IDs,`n,`n
	{
		...
		wb%WB_counter%.Navigate(game A_LoopField ".w3g")
		GoSub, wait
		...
		If temp not contains Can't read replay file
		{
			If (temp != "")
			{
				...
				content .= date " :: (Game) " gn " :: " game A_LoopField ".w3g`n" content_t "`n`n"
			}
		}

		wb%WB_counter%.Navigate(lobby A_LoopField ".txt")
		GoSub, wait

			content_T := wb%WB_counter%.document.documentElement.outerHTML
			If content_T not contains <title>HTTP 404:
				content .= date " :: (Lobby) " gn " :: " lobby A_LoopField ".txt`n" content_t "`n`n"
	}
...
Copy:
{
	If (A_GUI > 2 && A_EventInfo  != "0" && A_GuiEvent = "RightClick")
	{
		...
		If row_data contains http://atl.entgaming.net/replay/
			Clipboard := row_data
...
WM_LBUTTONDBLCLK(wParam,lParam)
{
	If (A_GUI > 2)
	{
	    ...
	    If RowText contains http://atl.entgaming.net/replay/
	    {
