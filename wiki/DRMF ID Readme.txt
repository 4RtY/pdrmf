...
Known bug: Although there's the reload option, Windows, the DRM or the DRMF itself either cause or don't update the log.txt's filesize. At the moment, I do not know how to solve this since I don't 100% know from what this is caused. To solve this, please restart the DRMF after each time you are loading a new actionlog which is somehow updating the log.txt's size (only needed if you started the DRMF before running the DRM's actionlog).
...
Macro
The program now contains an in-build c-macro. ctrl+s to start and ctrl+a to abort it. The speed can be set in the settings.ini.
...
SyncIgnore
Will maybe be added soon too. Will remove the first 200+ pre selection lines of the DRM filter chat.[/s]
...
Reload
Will reload the chat's content, the log file's size and the settings.ini. From now on you dont need to restart the exe, simply reload it. #FF4040][Remember the known bug]

#00BF00]DotA Replay Manager Filter Settings
Version
