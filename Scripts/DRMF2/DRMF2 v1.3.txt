...
{
	MsgBox,16,Info,You are using an unknown version of the DRM Filter.`nPlease delete the current settings.ini or wait till it's recreated!`n%settingspath%
	GoSub, Settings
	exitapp
}
...
reload:
{
	...
	{
	IfWinNotExist,ahk_class MainWndClass
		TrayTip,Chatlog could not be loaded.,Please start the DotA Replay Manager.,3,2

/*
	Loop, 50
	{
		ControlGetText,ButtonContent,Button%A_index%,DotA Replay Manager
		If (ButtonContent = "Load")
		{
			ButtonToClick := "Button" A_index
			ControlClick,%ButtonToClick%,DotA Replay Manager
			Loop, 2
...
Menu,MyMenu,add, DotA GUI, DotA
	IfExist,%A_workingdir%\icons\logo.ico
		Menu, MyMenu, Icon,  DotA GUI,%A_workingdir%\icons\logo.ico,,30x30
	else
		ErrorIcon := 1
}
If A_GUI = 3
{
	Menu,MyMenu,add, ID GUI, ID
		IfExist,%A_workingdir%\icons\logo.ico
		Menu, MyMenu, Icon, ID GUI,%A_workingdir%\icons\logo.ico,,30x30
	else
		ErrorIcon := 1
}
...
Menu,MyMenu,Add
Menu, MyMenu, Add, Readme, Readme
	IfExist,%A_workingdir%\icons\placeholder.ico
		Menu, MyMenu, Icon, Readme,%A_workingdir%\icons\placeholder.ico,,30x30
	else
		ErrorIcon := 1
...
user:
{
	GuiControlGet,user2,,Edit1
	IniWrite,%user2%,%settingspath%,Settings,user2
}
return
*/
...
Loop, 10
{
	ControlGetText,ReplayName,Edit%A_index%,ahk_class MainWndClass
	If ReplayName contains Warcraft III\Replay\
		break
}
RegRead,WarPath,HKEY_CURRENT_USER,Software\Blizzard Entertainment\Warcraft III,InstallPathX
StringReplace,GameName,replayname,%warpath%,,All
StringReplace,Gamename,gamename,\Replay\,,All

	ControlGet,Details,List,Col3,SysListView321,ahk_class MainWndClass
	Loop, parse, details, `n,`n
		Detail%A_index% := A_loopField

If (Detail5 = "")
	Detail5 := gamename
If (Detail5 = "") ; incase reg couldnt be read
	Detail5 = <Unknown> 
...
{
	If (detail%A_index% = "")
		detail%A_index% = <Unknown>
}
If (Detail5 = "<Unknown>")
	Detail5 := gamename

If Replayname contains Island Defense
	Detail3 = Island Defense
If Replayname contains DotA
	Detail3 = DotA
...
Loop, 11
	LV_ADD(%A_index%,"")
		...
		LV_Modify(8,"","Host name:",detail6)
		LV_Modify(9,"","Replay saver:",detail7)
		LV_Modify(10,"","Game length:",detail8)
		LV_Modify(11,"","Winner:",detail9)
		LV_ModifyCol()

		GuiControl,Move,SysListView321, % "w" A_ScreenWidth "h300"
		Gui,%test%: add, edit,x0 y300 +readonly -vscroll w%A_ScreenWidth% h%A_screenheight%,
...
If (FontSize = ERROR || FontSize = "")
	FontSizeC = 12
else
	FontSizeC := (FontSize + 1)

GuiControl, 7: choose,Combobox4,%FontSizeC%
...
Gui, 7: add, text, y440 x5, ClearClipboard:
Gui, 7: add, DDL, y440 x100 w120 r2 vclearclipboard,0|1
...
If clearclipboard = 1
	...
	GuiControl, 7: choose, combobox11,%clearclipboardC%

Gui, 7: add, text, y470 x5, HeroException:
Gui, 7: add, DDL, y470 x100 w120 r2 vDisableException,0|1
...
If DisableException = 1
 	...
 	GuiControl, 7: choose, combobox12,%DisableExceptionC%
	
Gui, 7: add, button, default gSubmit y500 x5 w120 h25, Save and Change
Gui, 7: add, button, gDefault y500 x150 w120 h25,Erase and Default
OnMessage(0x200, "WM_MOUSEMOVE")
Gui, 7: Show,h530 w500,Settings ; h550
}
...
If A_GUI = 7 ; settings gui
{
		...
		If (7Control = "Static15") ; ClearClipboard
		...
		If (7Control = "Static16") ; except for
...
^d::
ControlClick,Load,ahk_class MainWndClass
return