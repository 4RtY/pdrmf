...
; ControlGet, Text, List,, SysListView3211, DotA Replay Manager
...
/*
Storm (Panda) ItemID: n01M
Earth (Panda) ItemID: n01N
Fire (Panda) ItemID: n01O
*/
...
For more details on how the options work and which you can/should use, check out the <a HREF="https://code.google.com/p/pdrmf/">wiki</a> on the DRMF's site.
</p>
...
If (Name2 != "")
{
	...
	; slot usage
	If (Subgroup = "0" && Deto = "0" && commands = "0" && log = "0" && titan = "0" && builders = "0" && items = "1" && buildings = "0" && spells = "0" && Neutrals = "0" && change = "0" && private = "0" && allies = "0" && all = "0" && share = "0" && groundclicks = "0" && shift = "0" && ItemClicks = "0")
	{
		Loop, read, %logpath%
		{
			IfInString,A_loopreadline,%Name2%
...
StringReplace,UserActions,UserActions,ItemID: n01M,Storm (Panda),All
StringReplace,UserActions,UserActions,ItemID: n01N,Earth (Panda),All
StringReplace,UserActions,UserActions,ItemID: n01O,Fire (Panda),All
...
Gui, 2: add, checkbox, y80  x15  vItems         , Items use
...
URLdownloadToFile,http://pdrmf.googlecode.com/files/VersionCheckID.ini,%A_workingdir%\VersionCheckID.ini
IniRead,NewVersion,%A_workingdir%\VersionCheckID.ini,VersionCheck,NewVersion
IniRead,NewName,%A_workingdir%\VersionCheckID.ini,VersionCheck,NewName
IniRead,NewUrl,%A_workingdir%\VersionCheckID.ini,VersionCheck,NewUrl
IniRead,Comment,%A_workingdir%\VersionCheckID.ini,VersionCheck,Comment
	...
	FileDelete,VersionCheckID.ini	
		...
		If (NewVersion != version) and if (NewVersion > version) ; new
		{
			...
			Gui, 60: font, s12,
			Gui, 60: font, bold
			Gui, 60: Add, text,, A new DotA Replay Manager Filter for Island Defense Version is available.
			Gui, 60: Add, text, cGreen,Click here to open the information site.
			Gui, 60: show,h80 w620, Info - New Version %newversion%
...
IfWinExist,ahk_class #32770
{
	...
	If (gameplayers = "")
	{
		Msgbox,48,Info,The DotA Replay Manager has to be opened and if not already done, a replay has to be chosen to get the needed information for the DRM Filter's full functionality.`nError2
		return
	}
...
{
	MsgBox,48,Info,The current replayseeker path in the settings does not exist.`nPlease edit it.
	return
}
